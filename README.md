...
- [Skill 一览](#skill-一览)
- [辅助工具：md2pdf —— 把 .md 转成 PDF](#辅助工具md2pdf--把-md-转成-pdf)
- [谁适合用这个项目](#谁适合用这个项目)
- [前置准备](#前置准备)
- [安装与使用](#安装与使用)
- [几个关键设计决策](#几个关键设计决策)
...
### 5. [委托代理前案件分析 `dailiqian-anjian-fenxi`](./dailiqian-anjian-fenxi/)　✅

协助律师在正式接受委托代理前，对民商事诉讼案件进行系统性分析。覆盖起诉前评估、应诉、二审、再审全阶段，十个步骤依次推进：

材料阅读 → 客户身份与利益图谱 → 事实与法律关系梳理 → 法律法规检索与法条支撑 → 诉讼方案分析 → 争议焦点与论证强弱评估 → 交叉验证与矛盾识别 → 胜诉可能性预判 → 待核实事项与证据缺口 → 代理策略建议 → 代理建议
...
## 辅助工具：md2pdf —— 把 .md 转成 PDF

[md2pdf](./md2pdf/) 是一个通用命令行工具，将任意 Markdown 文件转为排版好的 PDF，支持中文排版优化。

### 使用方式

```bash
md2pdf /path/to/文件.md
```

也可输入 `md2pdf `（末尾带空格）后把文件从访达拖进终端窗口，路径自动填入，按回车即可。

配合 Claude Code 使用时，可直接说明"生成答辩状并转为 PDF"，Claude Code 会依次完成全部操作。

> 详细安装和使用说明见 [md2pdf/README.md](./md2pdf/README.md)。

---

## 谁适合用这个项目

- 正在使用或准备使用 AI 工具辅助法律工作的执业律师
- 企业法务人员，需要起草或审查合同、出具内部法律意见
- 法学生，希望通过 AI 辅助学习法律文书的结构和写法

本项目的 Skill 是一套结构化的法律工作流程规范，可配合多种 AI 工具使用，无需编程基础。
...
也可以把 `SKILL.md` 的内容直接作为系统提示词，复制到以下大模型的对话框中使用：
...
合同审查（hetong-shencha）需要操作 Word 文档，运行时依赖 Python 3 和 lxml 库。如环境中尚未安装，在 Claude Code 中输入：
...
```bash
brew install pandoc
# Chrome 从 https://www.google.com/chrome/ 下载安装
```

或在 Claude Code 中输入 `帮我安装 pandoc 和 Chrome`。详细说明见 [md2pdf/README.md](./md2pdf/README.md)。
...
1. 打开需要的 Skill 文件夹（如 [dabianzhuan/SKILL.md](./dabianzhuan/SKILL.md)）
2. 复制 `---` 分隔线以下的全部内容
3. 粘贴到 AI 工具的对话框中，作为第一条消息或系统提示词
4. 正常提供案件材料、开始对话
...
如果需要直接拿到 PDF 版本，用 [md2pdf](./md2pdf/) 一键转换即可。已针对中文排版做了优化（苹方字体、首行缩进、1.8 倍行距）。
